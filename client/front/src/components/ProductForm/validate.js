export default function validate({
  ...
  if (!lettersAndSymbols.test(description))
    errors.description = "Description contains invalid characters";
  if (description && description.length > 1000)
    errors.description = "Description cannot be longer than 1000 characters";

  if (!price) errors.price = "A price needs to be set";
  if (!integers.test(price))
    errors.price = "A positive integer number is required";
  if (price > 10000) errors.price = "Maximum price value is 10000 dollars";

  if (offert === undefined || offert === null || offert === "")
    errors.offert = "A discount percentage is required";
  if (!discount.test(offert))
    errors.offert = "Discount percentage must be a number between 0 and 100";
  if (offert < 0 || offert > 100)
    errors.offert = "Discount percentage must be a number between 0 and 100";
